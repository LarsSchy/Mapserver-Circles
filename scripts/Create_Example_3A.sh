#!/bin/bash
#
#	Create examaple 3A for circles in Mapserver
# 
...
CIRC_SQL_VRT=../data/points_3857.vrt

IMAGE=../images/Circle_example-3A.png
...
EOF
	...
	#<OGRVRTLayer name='full_circle_stripe'>
		#<SrcDataSource relativeToVRT="1">points_3857.csv</SrcDataSource>
		#<LayerSRS>EPSG:3857</LayerSRS>
		#<SrcSQL dialect='sqlite'>
		#SELECT height, distance,label_1,label_2,
              #MakeCircularStripe(CAST(east AS float),CAST(north AS float), 200000, CAST(distance as float), 0.000001, 359.999999, 3857) as Geometry
		#FROM points_3857
     #</SrcSQL>
     #</OGRVRTLayer>
