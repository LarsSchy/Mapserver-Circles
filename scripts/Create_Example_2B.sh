#!/bin/bash
#
#	Create examaple 2B for circles in Mapserver
# 
...
CIRC_STR_CSV=../data/point_circ_str_label.csv

IMAGE=../images/Circle_example-2B.png
...
echo "ID,WKT,LABELTXT" > ${CIRC_STR_CSV}
...
grep "\\S" --color=none | \
cut -d "," -f 2,3,4,5 | \
while IFS=, read -r east north radius label
do 
    north_min=$(echo "${north}-${radius}" | bc)
    north_max=$(echo "${north}+${radius}" | bc)
cat << EOF >> ${CIRC_STR_CSV}
${ID},"CIRCULARSTRING(${east} ${north_min}, ${east}  ${north_max}, ${east}  ${north_min})","${label}"
EOF
